"""Idempotent migration runner — runs at container startup."""
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://adega:adega_secret@postgres:5432/adega")

# ---------------------------------------------------------------------------
# Embedded schema (mirrors /init.sql — keeps api/ self-contained for Railway)
# ---------------------------------------------------------------------------
SCHEMA_SQL = """
...
('encorpado', 'Encorpado'), ('leve', 'Leve'), ('frutado', 'Frutado'),
('seco', 'Seco'), ('doce', 'Doce'), ('premiado', 'Premiado'),
('reserva', 'Reserva'), ('organico', 'Orgânico'), ('promocao', 'Promoção'),
('presente', 'Bom para presente'), ('custo-beneficio', 'Custo-benefício'),
('jantar', 'Ideal para jantar'), ('massa', 'Harmoniza com massas'),
...
MIGRATION_002_SQL = """
ALTER TABLE wines
    ADD COLUMN IF NOT EXISTS sweetness_level TEXT DEFAULT 'seco',
    ADD COLUMN IF NOT EXISTS body TEXT DEFAULT 'medio',
    ADD COLUMN IF NOT EXISTS on_promotion BOOLEAN DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS knowledge_level TEXT DEFAULT 'intermediario',
...
QUIZ_DEFAULT_CONFIG = [
    {
        ...
        "options": [
            ...
            {"value": None, "label": "Só o vinho, sem comida", "emoji": "🍷"},
        ],
    },
    {
        "step_id": "price",
    ...
    },
    {
        ...
        "options": [
            ...
            {"value": "suave", "label": "Suave — mais leve e frutado", "emoji": "🌸"},
...
async def is_applied(conn: asyncpg.Connection, name: str) -> bool:
    return bool(await conn.fetchval("SELECT 1 FROM _migrations WHERE name=$1", name))


async def mark_applied(conn: asyncpg.Connection, name: str) -> None:
    await conn.execute(
        "INSERT INTO _migrations (name) VALUES ($1) ON CONFLICT DO NOTHING", name
    )


async def run_migration(conn: asyncpg.Connection, name: str, sql: str) -> None:
    if await is_applied(conn, name):
        log.info("%s: already applied", name)
        return
    log.info("%s: applying...", name)
...
async def main() -> None:
    log.info("Connecting to %s", DATABASE_URL[:DATABASE_URL.find("@") + 1] + "***")
    max_attempts = 10
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt == max_attempts:
                log.error("Could not connect after %d attempts: %s", max_attempts, e)
                sys.exit(1)
            wait = min(2 ** attempt, 30)
            log.warning("Connection failed (attempt %d/%d), retrying in %ds...", attempt, max_attempts, wait)
            await asyncio.sleep(wait)

    try:
        await ensure_migration_table(conn)

        wines_exists = await conn.fetchval(
            "SELECT 1 FROM information_schema.tables WHERE table_name='wines' AND table_schema='public'"
        )
        if wines_exists:
            log.info("001_init: schema already exists, marking done")
            await mark_applied(conn, "001_init")
        else:
            await run_migration(conn, "001_init", SCHEMA_SQL)

        await run_migration(conn, "002_extended", MIGRATION_002_SQL)
        await run_migration(conn, "003_settings", MIGRATION_003_SQL)

        # Seed default quiz config if missing
        exists = await conn.fetchval("SELECT 1 FROM settings WHERE key='quiz_config'")
        if not exists:
            await conn.execute(
                "INSERT INTO settings (key, value) VALUES ('quiz_config', $1::jsonb)",
                json.dumps(QUIZ_DEFAULT_CONFIG),
            )
            log.info("quiz_config: default seeded")
